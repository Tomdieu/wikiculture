...
const Sidebar = (props: Props) => {
  ...
  const cultural_area = searchParams.get("cultural_area") || "";
  ...
  useEffect(() => {
    ...
    if (cultural_area) {
      filters.addCulturalArea(cultural_area);
    }
    if (region) {
      filters.getCulturalAreas().then((culturalAreas) => {
        const filteredCulturalArea = culturalAreas.find((cA) =>
          cA.regions.find((re) => re.name === region)
        );
        console.log({ filteredCulturalArea });
        if (filteredCulturalArea && !filters.regions.includes(region)) {
          filters.addCulturalArea(filteredCulturalArea?.name);
          filters.addRegion(region);
        }
      });
    }
    if (village) {
      filters.getCulturalAreas().then((culturalAreas) => {
        const filteredCulturalArea = culturalAreas.find((cA) =>
          cA.regions.find((re) => re.villages.find((vi) => vi.name === village))
        );
        const _culturalAreas = culturalAreas.find((cA) =>
          cA.regions.find((re) => re.villages.find((vi) => vi.name === village))
        );
        const _region = _culturalAreas?.regions.find((re) =>
          re.villages.find((vi) => vi.name === village)
        );

        if (filteredCulturalArea) {
          filters.addCulturalArea(filteredCulturalArea?.name);
          if(_region){

            filters.addRegion(_region?.name);
            }
          filters.addVillage(village);
        }
      });
    }
  }, [category,cultural_area,village,region]);
  ...
  return (
    <div className="min-h-full w-full h-full flex flex-col space-y-2 p-3 sticky top-0 bottom-0 overflow-y-auto">
        ...
        {filters.isToClear && (
          <div className="flex items-center space-x-1">
            <div
              ...
              className="flex items-center space-x-1 rounded-full border px-1 py-1 text-muted-foreground select-none cursor-pointer"
            >
      ...
      <div>
        ...
        <div className="flex flex-col space-y-1">
          ...
          {cultural_areas?.map((cultural_area, index) => (
            ...
            >
              <Checkbox
                id={cultural_area.name}
                checked={filters.cultural_areas.includes(cultural_area.name)}
                onCheckedChange={(checked) => {
                  if (checked) {
          ...
          <div className="flex flex-col space-y-1">
            ...
            <div className="flex flex-col space-y-1 relative">
              <Popover open={isOpen} onOpenChange={setIsOpen}>
                ...
                <PopoverContent
                  align="end"
                  className="w-full p-0 max-h-[500px] overflow-auto"
                >
                  <Command className="relative top-0 max-h-[400px] overflow-auto">
                    ...
                    <CommandGroup
                      forceMount
                      value="villages"
                      heading={`Villages (${villages?.length})`}
                    >
              ...
              <div className="flex flex-wrap space-x-2 space-y-1">
                {filters.villages?.map((village, index) => (
                  <div
                    key={index}
                    className="flex items-center space-x-2 rounded-full border px-2 py-1 text-muted-foreground select-none"
                  >
                    <span className="text-xs">{village}</span>
                    <div className="border rounded-full p-1 cursor-pointer">
                      <X
                        className="w-3 h-3"
                        onClick={() => filters.removeVillage(village)}
                      />
                    </div>
                  </div>
                ))}
