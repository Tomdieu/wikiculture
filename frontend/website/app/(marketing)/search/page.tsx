...
const page = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (data && query) {
      data.results.forEach((article) => {
        ...
        if (contentElement) {
          contentElement.innerHTML = highlightText(
  ...
  const highlightText = (text: string, searchTerm: string) => {
    ...
    return text.replace(
      regex,
      '<span class="bg-yellow-500 font-bold select-text">$1</span>'
    );
  };
    ...
    <div className="flex items-center flex-col w-full h-full space-y-3 py-3 container mx-auto min-h-full">
      <div>
        ...
        <div className="space-y-6">
          {data && data?.count && (
            <p>
              Found {data?.count} results containing : {query}
            </p>
          )}
              ...
              >
                ...
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: cleanHtml(article.content),
                  }}
                />
              </Link>
            ))}
