        ...
        cell({ row }) {
          ...
          return (
            <p className="font-medium text-ellipsis overflow-hidden line-clamp-1">
              {DOMPurify.sanitize(village.description)}
            </p>
          );
        },
      },
        ...
        cell: ({ row }) => {
          ...
          return (
            <Dialog onOpenChange={() => {}}>
              ...
              <DialogContent
                className={cn("w-full lg:max-w-5xl p-0 overflow-y-auto prose max-w-fit",{"h-full":village.description.length > 200})}
                forceMount
              >
                {village.description.length === 0 ? (
